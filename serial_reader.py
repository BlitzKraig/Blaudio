...
class SerialReader:
    def __init__(self, port, callback, message_callback, baudrate=9600,
                 callback_interval=0.02, smoothing_window=10,
                 old_range=(0, 1023), new_range=(0, 100), retry_interval=5):
                ...
                if self.ser.in_waiting > 0:
                    ...
                    try:
                        ...
                        if version_line < MINIMUM_HARDWARE_VERSION:
                            msg = 'Incompatible HW. Current: {}. Required: {}.'.format(
                                version_line, MINIMUM_HARDWARE_VERSION)
                            print(msg)
                        ...
                        for i, value in enumerate(knob_values):
                            ...
                            new_avg = (old_avg - self.old_min) / (self.old_max - self.old_min) \
                                      * (self.new_max - self.new_min) + self.new_min
                            self.knobs[i] = int(np.rint(new_avg))
